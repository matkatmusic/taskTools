...
7b. Invoke AskUserQuestion to ask whether to customize the model or effort for any of this task's prompt-generating phases, offering an option to skip and use the defaults. If the user skips, omit the \`agent\` field entirely. Otherwise, invoke AskUserQuestion again with two multiSelect questions — one offering PLAN_THE_TASK, IMPLEMENT_TASK, and FIX_IMPLEMENT_TASK_TESTS, the other offering FIX_CONFLICTS and FIX_THE_CODEBASE_FOR_SUITE — and record every phase the user selects; these five are the only phases whose agent can be overridden. For each selected phase, invoke AskUserQuestion once more to ask for its \`model\` (e.g. claude-opus-5[1m], claude-sonnet-5[1m], claude-fable-5-1[1m]) and its \`effort\` (high, medium, low), letting the user type a different model or effort through the Other option. Populate an \`agent\` object keyed by each selected phase name with its chosen \`{ "model": ..., "effort": ... }\`, omitting any phase the user did not select.

8. Pipe the merged payload into \`appendTask.ts\`, combining the user-settled \`title\`, \`userDescription\`, \`goal\`, \`hasTests\`, \`tests\`, \`schemaVersion\`, \`problemSolvedByTask\`, \`notInScope\`, and optional \`chainGoal\` and \`agent\` with the workflow's \`files\`, \`createsFiles\`, \`description\`, \`difficulty\`, and \`blockedBy\`. Populate \`userDescription\` with the task description verbatim, exactly as typed — never edit, summarize, or reword it. If the request names the source note/handoff file(s) the task came from (e.g. an \`update-tasks\` harvest), also include \`"handoffFilePaths": [<those repo-relative paths>]\` in the object; otherwise omit the field. Run this with Bash, with the payload filled in:
