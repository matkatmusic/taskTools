...
export function isStagingAncestorOfWorktree(worktree: string): boolean {
...
function retainedDestination(worktree: string, own: Record<string, unknown>): (RebaseResumedPacket & { next: string }) | null {
  ...
  if (intent === null)
    return null;
  clearRebaseIntent(worktree);
  return { ...JSON.parse(intent.targetInput), ...own, next: intent.targetBlock } as RebaseResumedPacket & { next: string };
}
