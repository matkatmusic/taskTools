// Behavioral checks for CODEX_REVIEW_FALLBACK_FABLE.ts: retries the review question with claude fable only, no chain.
import { test } from "node:test";
import assert from "node:assert/strict";
import { mkdtempSync, mkdirSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { main } from "./CODEX_REVIEW_FALLBACK_FABLE.ts";

process.env.RUN_STEP_LOG = join(tmpdir(), "codex-review-fallback-fable-run-log.json");

function makeFixture(): { worktree: string; projectRoot: string } {
    const repoRoot = mkdtempSync(join(tmpdir(), "codex-review-fallback-fable-"));
...
function packetFrom(fixture: ReturnType<typeof makeFixture>) {
    return {
        box: "DID_CODEX_REVIEW_SUCCEED_Q", scriptSignal: "continue", taskNumber: 42, runId: "run-1",
...
test("test_main_returnsAPromptSignal", () => {
    ...
    assert.equal(output.box, "CODEX_REVIEW_FALLBACK_FABLE");
...
test("test_main_tellsTheAgentWhetherTheCommandSucceeded", () => {
    const output = main(JSON.stringify(packetFrom(makeFixture())));
    assert.match(output.prompt as string, /"fableSucceeded"/);
});
