...
export async function main(input: string): Promise<Record<string, unknown>> {
  ...
  // );
  // ponytail: touched tests only; the full suite is the staging merge gate, not a per-task gate.
  const result = await runTaskTests(packet.taskNumber, packet.runId, packet.worktree, `run-full-suite-${attempts}`, packet.projectRoot);
  return {
    ...packet,
    box: "RUN_FULL_SUITE",
