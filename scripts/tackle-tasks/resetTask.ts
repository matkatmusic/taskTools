...
import { isStagingAncestorOfWorktree } from "./preambleStatusCheck/REBASE_RESUMED_WORKTREE_ONTO_STAGING.ts";
import { writeRebaseIntent } from "./shared/rebaseIntent.ts";
...
export async function resetTask(taskNumber: number, block: string): Promise<string> {
  ...
  if (block !== "") {
    ...
    if (!stagingExists || isStagingAncestorOfWorktree(worktreePath)) {
      ...
      writeCheckpoint(worktreePath, {
        taskNumber, passId: randomUUID(), runId, projectRoot: repoRoot,
        block: rebaseBlockKey, input: rebaseInput, state: "running",
        sourceLockHeld: reachedFromLock.has(rebaseBlockKey), exitType: "", exitNote: "", resumedFrom: null,
      });
      lines.push(`staging moved since task ${taskNumber}'s worktree was cut; it rebases onto staging before resuming at ${stepKey} on the next /tackle-tasks [${taskNumber}]`);
    }
  ...
  if (block === "")
    rmSync(dirname(stepsConfigPath), { recursive: true, force: true });
  return lines.join("\n");
}
