...
import { notesFile, requiredTestGroups, taskDeclaresTests, writableFiles as ownerWritableFiles } from "./writableFiles.ts";
...
export type PreparedTask = {
  ...
  // Everything the implementer may write, repo-relative: modifiableFiles, required test candidates, and the notes file.
  writableFiles: string[];
  // One group per owned source file: its repo-relative test candidates. [] when the task declares no tests.
  requiredTestGroups: { source: string; candidates: string[] }[];
...
export function loadPreparedTask(taskNumber: number, worktree: string, projectRoot: string): PreparedTask {
  ...
  if (!task)
    fail(`task ${taskNumber} not found in tasks.json`);
  ...
  for (const path of readOnlyFilePaths) {
    if (!existsSync(path))
      throw new Error(`task ${taskNumber}: readOnlyFiles names ${path} but the file does not exist`);
  }
  ...
  for (const path of handoffPaths) {
    if (!existsSync(path))
      throw new Error(`task ${taskNumber}: handoffFilePaths names ${path} but the file does not exist`);
  }
  const createsFiles: string[] = Array.isArray((task as any).createsFiles) ? (task as any).createsFiles : [];
  for (const file of files) {
    if (createsFiles.includes(file))
      continue;
  ...
  return {
    ...
    notesFile: `${worktree}/${notesFile(taskNumber)}`,
    files,
    readOnlyFiles: declaredReadOnlyFiles,
    ownedFilePaths: files.map((file) => `${root}/${file}`),
    writableFiles: ownerWritableFiles(task),
    requiredTestGroups: requiredTestGroups(task),
    ...
    testFilePaths: requiredTestGroups(task)
      .flatMap((group) => group.candidates)
      .map((candidate) => `${root}/${candidate}`)
      .filter((path) => existsSync(path)),
    hasTests: taskDeclaresTests(task),
