...
export async function runTaskTests(
  ...
  for (const occurrence of occurrences) {
    ...
    for (const change of runnable) {
      ...
      if (change.kind === "A")
        createdTestFiles.push(taggedPath);
    }
  ...
  if (testFiles.length === 0 || absentPairedTests.length > 0) {
    ...
    passed = !missingTests;
    if (missingTests)
      outputParts.push(`the task declares tests but the branch lacks: ${absentPairedTests.join(", ")}`);
  }
  else {
    missingTests = false;
    ...
    if (suite.timedOut)
      outputParts.push(`the branch's test suite timed out after ${timeoutMs}ms and was killed`);
