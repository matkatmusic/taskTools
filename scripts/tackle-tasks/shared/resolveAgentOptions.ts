...
// One per task per band block: a Bash hook blocks git and the full suite, an Edit/Write hook denies paths outside the box fence.
function writeFencedAgentFile(tasksFile: string, taskNumber: number, box: string): string {
    ...
    // disallowedTools keeps only the tool name of "Bash(git *)" and would remove Bash entirely; the Bash hook filters commands instead.
