// Behavioral checks for scripts/tackle-tasks/closeTaskRun.ts.  Run: node --test tests/closeTaskRun.test.ts
...
test("test_closeTaskRun_archivesAndUnblocksInOneCall", () => {
    ...
    // Verification: closed/skipped/unblocked all come back from the one call, task 1 is archived with hashes derived from its own run, and task 2 no longer lists it as a blocker.
...
test("test_closeTaskRun_archiveFirstRetryReusesTheStoredNoteAndHashesInsteadOfOverwriting", () => {
    // Setup: simulates a failure after the completed-file write but before the tasks.json removal — task 1 is present in BOTH files. The open copy's run carries a DIFFERENT commit hash than what's archived, and the archived record carries extra durable fields a re-derivation would never reproduce — proving the archive isn't re-derived.
    ...
    // Test action: retry the close with a deliberately different note (and no way to even supply different hashes now — the contract no longer accepts them).
    const output = closeTaskRun({
        taskNumber: 1, runId: "run-a", closureNote: "a completely different, wrong note", projectRoot: root, stepId: "step-1",
    });

    // Verification: the open record is removed and dependents unblocked, but completedTasks.json is untouched byte-for-byte — not merely field-equal.
...
test("test_closeTaskRun_archiveFirstRetryUnblocksDependentsWhileLeavingTheArchiveByteIdentical", () => {
    // Setup: same archive-first-retry shape, but with a dependent task blocked on task 1 — proving unblocking still runs even though the archive write is skipped.
...
test("test_closeTaskRun_reconcilesSuccessAfterACompleteSuccessWithoutRewrite", () => {
    // Setup: task 1 already fully closed — present only in completedTasks.json — with a record that matches exactly what this call would have produced.
    const runState = completedRunState();
    const archivedRecord = {
        taskNumber: 1, title: "finished", run: runState,
...
test("test_closeTaskRun_reportsAmbiguityWhenTheOnlyArchivedRecordDoesNotMatchThisCall", () => {
    // Setup: task 1 is archived, but this call's note disagrees with what is stored — never report success, never overwrite. Ambiguity must be named distinctly from "not found": evidence exists here, it just disagrees, unlike a task absent from both files.
    const runState = completedRunState();
    const archivedRecord = {
        taskNumber: 1, title: "finished", run: runState,
        completionDate: "2026-08-01", closureNote: "the real note", commitHashes: ["abc123"],
