// Behavioral checks for RECORD_MERGE_COMMIT_HASHES.ts.
...
function samplePacket(projectRoot: string, taskNumber: number, runId: string, commits: unknown[]): Record<string, unknown> {
    return {
        box: "MERGE_SUCCEEDED_EXIT", scriptSignal: "continue", projectRoot, taskNumber, runId,
        worktree: `${projectRoot}/.worktrees/task-${taskNumber}`, branch: "main",
...
test("test_RECORD_MERGE_COMMIT_HASHES_carriesTheRestOfThePacketForward", () => {
    ...
    assert.equal(output.worktree, `${projectRoot}/.worktrees/task-${taskNumber}`);
