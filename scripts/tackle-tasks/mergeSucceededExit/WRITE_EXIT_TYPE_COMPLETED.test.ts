// Behavioral checks for WRITE_EXIT_TYPE_COMPLETED.ts.
...
import { main } from "./WRITE_EXIT_TYPE_COMPLETED.ts";
...
const TEMPLATE_PATH = join(dirname(fileURLToPath(import.meta.url)), "WRITE_EXIT_TYPE_COMPLETED.template.json");
...
function samplePacket(projectRoot: string, taskNumber: number, runId: string, worktree: string): Record<string, unknown> {
    return {
        ...
        worktree, branch: "main", exitNote: "All layers merged successfully.",
    };
}
...
test("test_WRITE_EXIT_TYPE_COMPLETED_carriesTheRestOfThePacketForward", () => {
    ...
    assert.equal(output.worktree, "/tmp/worktree");
