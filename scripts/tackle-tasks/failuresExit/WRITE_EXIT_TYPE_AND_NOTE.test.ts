// Behavioral checks for WRITE_EXIT_TYPE_AND_NOTE.ts. Run: node --test scripts/tackle-tasks/failuresExit/WRITE_EXIT_TYPE_AND_NOTE.test.ts
import { test } from "node:test";
import assert from "node:assert/strict";
import { mkdtempSync, readFileSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { main } from "./WRITE_EXIT_TYPE_AND_NOTE.ts";
import { readTaskRunState } from "../shared/taskRunState.ts";
import { getTemplateShapeMismatches } from "../../templateShape.ts";

const TEMPLATE_PATH = join(import.meta.dirname, "WRITE_EXIT_TYPE_AND_NOTE.template.json");
