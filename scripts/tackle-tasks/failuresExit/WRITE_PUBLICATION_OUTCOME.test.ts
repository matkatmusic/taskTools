// Behavioral checks for WRITE_PUBLICATION_OUTCOME.ts. Run: node --test scripts/tackle-tasks/failuresExit/WRITE_PUBLICATION_OUTCOME.test.ts
import { test } from "node:test";
import assert from "node:assert/strict";
import { mkdtempSync, readFileSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { main } from "./WRITE_PUBLICATION_OUTCOME.ts";
import { readTaskRunState } from "../shared/taskRunState.ts";
import { getTemplateShapeMismatches } from "../../templateShape.ts";

const TEMPLATE_PATH = join(import.meta.dirname, "WRITE_PUBLICATION_OUTCOME.template.json");
...
function packet(projectRoot: string) {
    return JSON.stringify({
        box: "DID_ANY_WORK_LAND_Q", scriptSignal: "continue", next: "WRITE_PUBLICATION_OUTCOME",
        taskNumber: 1, runId: "run-a", projectRoot, worktree: join(projectRoot, "worktree"),
        branch: "task-1", exitType: "run-failed", exitNote: "boom", publicationState: "SOME LANDED",
    });
}
