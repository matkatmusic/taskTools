// Behavioral checks for DOES_RUN_HOLD_LEASE_Q.ts. Run: node --test scripts/tackle-tasks/failuresExit/DOES_RUN_HOLD_LEASE_Q.test.ts
import { test } from "node:test";
import assert from "node:assert/strict";
import { mkdtempSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { main } from "./DOES_RUN_HOLD_LEASE_Q.ts";

function packet(worktree: string, runId: string) {
    return JSON.stringify({
        box: "MARK_TASK_INACTIVE_FAILURE", scriptSignal: "continue", taskNumber: 1, runId,
        projectRoot: "/repo", worktree, branch: "task-1", exitType: "run-failed", exitNote: "boom",
        publicationState: "NONE LANDED", modifiedFiles: [], active: false, endedAt: "2026-08-01T00:00:00-07:00",
    });
}

test("test_DOES_RUN_HOLD_LEASE_Q_choosesReleaseWhenThisRunOwnsTheLease", () => {
    const root = mkdtempSync(join(tmpdir(), "doesRunHoldLeaseQ-"));
...
test("test_DOES_RUN_HOLD_LEASE_Q_choosesSourceLockCheckWhenNoLeaseExists", () => {
    const root = mkdtempSync(join(tmpdir(), "doesRunHoldLeaseQ-"));
    const worktreePath = join(root, "worktree-1");

    const output = main(packet(worktreePath, "run-a"));

    assert.equal(output.next, "DOES_RUN_HOLD_SOURCE_LOCK_Q");
});

test("test_DOES_RUN_HOLD_LEASE_Q_choosesSourceLockCheckWhenAnotherRunOwnsTheLease", () => {
    const root = mkdtempSync(join(tmpdir(), "doesRunHoldLeaseQ-"));
    const worktreePath = join(root, "worktree-1");
