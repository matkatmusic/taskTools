// Behavioral checks for CODEX_TEST_REVIEW_FALLBACK_FABLE.ts: retries the review question with claude fable only, no chain.
...
import { main } from "./CODEX_TEST_REVIEW_FALLBACK_FABLE.ts";

process.env.RUN_STEP_LOG = join(tmpdir(), "codex-test-review-fallback-fable-run-log.json");

const SOURCE_BRANCH = "main";

function makePacket() {
    const projectRoot = mkdtempSync(join(tmpdir(), "codex-test-review-fallback-fable-"));
    ...
    return {
        box: "DID_CODEX_TEST_REVIEW_SUCCEED_Q", scriptSignal: "continue",
...
test("test_main_returnsAPromptSignal", () => {
    ...
    assert.equal(output.box, "CODEX_TEST_REVIEW_FALLBACK_FABLE");
...
test("test_main_tellsTheAgentWhetherTheCommandSucceeded", () => {
    const prompt = main(JSON.stringify(makePacket())).prompt as string;
    assert.match(prompt, /"fableSucceeded"/);
});
