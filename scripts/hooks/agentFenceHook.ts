...
import { writableFiles } from "../tackle-tasks/shared/writableFiles.ts";
...
if (typeof filePath !== "string")
  throw new Error(`agentFenceHook: ${input.tool_name} carries no file_path`);

const task = readTaskFile(tasksFile).find((entry) => entry.taskNumber === taskNumber);
if (task === undefined)
  throw new Error(`agentFenceHook: task ${taskNumber} not found in ${tasksFile}`);
...
// const widenedFiles = [...new Set([...files, ...pairedTestPaths, `plans/implementation-notes-${taskNumber}.md`])];
const fence = box === "PLAN_THE_TASK" ? ["plans/plan.json"] : writableFiles(task);
