...
import type { ResolutionManifest } from "../shared/resolutionRequests.ts";
...
export function extractEditedFiles(toolCalls: ToolCall[]): string[] {
  ...
  for (const call of toolCalls) {
    if (!FILE_MODIFYING_TOOLS.has(call.tool_name ?? ""))
      continue;
...
// Where a source file's test lives; [] means it is a test file or an unknown language.
export function testFileCandidates(occurrenceCwd: string, filePath: string): string[] {
  const langConfig = LANGUAGES[extname(filePath)];
  if (!langConfig)
    return [];
  const name = basename(filePath, extname(filePath));
  if (langConfig.isTest(name))
    return [];
  return langConfig.candidates(occurrenceCwd, name, dirname(filePath));
}
...
function findTestFile(
  ...
  const candidates = testFileCandidates(occurrenceCwd, filePath);
  for (const candidate of candidates) {
    if (existsSync(candidate))
      return { testFile: candidate };
  }
...
export function groupEditsByOccurrence(
  ...
  for (const filePath of editedFiles) {
    ...
    if (!extBatch.tests.includes(found.testFile))
      extBatch.tests.push(found.testFile);
...
export function runRelatedTests(
  ...
  for (const batch of batches.values()) {
    ...
    if (batch.byExtension.has(".ts") && existsSync(join(occurrenceCwd, "tsconfig.json"))) {
      ...
      if (typeErrors !== "")
        warnings.push(`Type errors after editing ${batch.byExtension.get(".ts")!.sources.join(", ")}:\n${typeErrors}`);
    }
  }
  return warnings;
}
...
function main(): void {
  ...
  if (!hookInput.session_id)
    throw new Error("relatedTests hook input requires session_id");
  ...
  for (const file of editedFiles) {
    ...
    if (spawnSync("git", ["-C", dirname(file), "rev-parse", "--show-toplevel"], { encoding: "utf8" }).status !== 0)
      continue;
