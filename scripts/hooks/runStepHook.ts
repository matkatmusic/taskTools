...
type Step = StepConfigEntry & { diagram: string; nextBlock?: string; translator?: string };
type StepRun = {
  ok: boolean;
  box: string;
  startedAt: number;
...
function walkFromStep(startStepKey: string, startInput: string, invocation: string): HookOutput {
  ...
  if (startPacket.taskNumber !== undefined)
    currentTaskNumber = Number(startPacket.taskNumber);
  ...
  if (typeof startPacket.packetFile === "string") {
    if (!process.env.RUN_STEP_LOG)
      runDirectory = dirname(dirname(startPacket.packetFile));
    ...
    if (packet.taskNumber !== undefined)
      currentTaskNumber = Number(packet.taskNumber);
  ...
  if (startStepKey !== START_STEP_KEY && typeof startPacket.tasksFile === "string") {
    ...
    if (entry === null)
      return walkFromStep(START_STEP_KEY, startInput, invocation);
  ...
  while (true) {
    ...
    if (!inFailureChain && worktreeExists && !step.producesPrompt && !answersAPrompt) {
      ...
      writeCheckpoint(worktree, {
        ...
        input,
        state: "running",
        sourceLockHeld: isInsideSourceLock(stepKey) === SOURCE_LOCK_REACHABLE,
        exitType: "",
        exitNote: "",
        resumedFrom: existing?.resumedFrom ?? null,
      });
    ...
    // An overridden hop may name a translator to reshape the payload; with none, the payload forwards verbatim.
    const nextPayload = step.nextBlock !== undefined && step.translator !== undefined
      ? runTranslator(step.translator, resultWithoutNext)
      : resultWithoutNext;
