// Runs the suite for /run-full-suite, typed as a prompt or invoked as the skill by an agent.
...
// Plugin skills reach the hook namespaced, as /taskTools:run-full-suite and taskTools:run-full-suite.
...
const args = prompt.startsWith("/run-full-suite ")
  ? prompt.slice("/run-full-suite".length)
...
if (tokens.length !== 5) {
  inject(`run-full-suite: expected 5 arguments, got ${tokens.length}: ${tokens.join(" ")}`);
  process.exit(0);
}

// The diagram box label, so a reconciled step result names the box it came from.
const result = runFullSuite(Number(taskNumber), runId, worktree, sourceBranch, "run the full suite", projectRoot);
