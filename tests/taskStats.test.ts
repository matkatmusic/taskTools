...
const FLAT_MANIFEST: RepositoryManifest = {
...
test("counts open and completed tasks separately", () => {
    const stats = computeTaskStats([openTask(1), openTask(2)], [closedTask(3, "2026-07-30")], TODAY, FLAT_MANIFEST);
...
test("a task is blocked only by blockers that are still open", () => {
    ...
    const stats = computeTaskStats(open, [], TODAY, FLAT_MANIFEST);
...
test("closure windows count completionDate within 7 and 30 days of today", () => {
    ...
    const stats = computeTaskStats([], completed, TODAY, FLAT_MANIFEST);
...
test("busiestDay is null when nothing has been closed", () => {
    assert.equal(computeTaskStats([], [], TODAY, FLAT_MANIFEST).busiestDay, null);
});
...
test("counts completed tasks that recorded commit hashes", () => {
    ...
    assert.equal(computeTaskStats([], completed, TODAY, FLAT_MANIFEST).completedWithCommitHashes, 1);
});
...
test("formatted output names every headline number", () => {
    const text = formatTaskStats(computeTaskStats([openTask(1, { files: ["a.ts"] })], [closedTask(2, "2026-07-30")], TODAY, FLAT_MANIFEST));
