...
test("test_suiteFixPrompt_derivesTheFailingOutputFromTaskRunStateNotFromTheCaller", () => {
    ...
    const prompt = suiteFixPrompt(makeTask(makeProjectRoot(RED)));
...
test("test_suiteFixPrompt_throwsWhenNoFullSuiteRunIsRecorded", () => {
    ...
    assert.throws(() => suiteFixPrompt(makeTask(makeProjectRoot(null))), /no recorded full-suite run/);
});
...
test("test_suiteFixPrompt_throwsWhenTheRecordedSuitePassed", () => {
    ...
    assert.throws(() => suiteFixPrompt(makeTask(makeProjectRoot(green))), /passed; this box runs only on a red suite/);
});
