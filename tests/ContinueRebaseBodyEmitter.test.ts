// Behavioral checks for scripts/tackle-tasks/ContinueRebaseBodyEmitter.ts. Run: node --test tests/ContinueRebaseBodyEmitter.test.ts
import { test } from "node:test";
import assert from "node:assert/strict";
import { continueRebasePrompt } from "../scripts/tackle-tasks/ContinueRebaseBodyEmitter.ts";
...
test("test_continueRebasePrompt_passesEveryHookArgumentQuotedAndInOrder", () => {
    // The hook refuses any count but five, and a worktree path with spaces needs the quotes.
    const prompt = continueRebasePrompt(task, "r1", "main");
    assert.match(prompt, /\/continue-rebase "35" "r1" "\/wt" "main" "\/project"/);
});

test("test_continueRebasePrompt_leavesTheVerdictToTheSkillAndAsksForItVerbatim", () => {
