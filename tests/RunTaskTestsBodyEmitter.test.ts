// Behavioral checks for scripts/tackle-tasks/RunTaskTestsBodyEmitter.ts. Run: node --test tests/RunTaskTestsBodyEmitter.test.ts
import { test } from "node:test";
import assert from "node:assert/strict";
import { runTaskTestsPrompt } from "../scripts/tackle-tasks/RunTaskTestsBodyEmitter.ts";
...
test("test_runTaskTestsPrompt_passesEveryHookArgumentQuotedAndInOrder", () => {
    // The hook refuses any count but five, and a worktree path with spaces needs the quotes.
    const prompt = runTaskTestsPrompt(task, "r1", "main");
    assert.match(prompt, /\/run-task-tests "35" "r1" "\/wt" "main" "\/project"/);
});

test("test_runTaskTestsPrompt_leavesTheVerdictToTheSkillAndAsksForItVerbatim", () => {
