...
test("test_mermaidForFile_parsesFunctionNamesAndChainsCallStatements", () => {
    ...
    assert.equal(
        result,
        '%%{init: {"flowchart": {"wrappingWidth": 100000}}}%%\nflowchart TD\n' +
        'B_f_ts["f.ts"]\n' +
        'B_a1["a()"]\n' +
        'B_a2["a()"]\n' +
        'B_a3["a()"]\n' +
...
test("test_mermaidForFile_forOfLoopBecomesADiamondWithABackEdge", () => {
    ...
    assert.equal(
        ...
        'B_total_equals_0["let total = 0"]\n' +
        'B_total_plus_equals_x["total += x"]\n' +
...
test("test_mermaidForFile_classicForConditionAndItsFlippedOppositeBecomeTheChoiceLabels", () => {
    ...
    assert.equal(
        ...
        'B_count["count()"]\n' +
        'B_log_i["log(i)"]\n' +
        'Q_CHOICE_for_i_equals_0_i_less_than_3_i_N["i >= 3"]\n' +
        'Q_CHOICE_for_i_equals_0_i_less_than_3_i_Y["i < 3"]\n' +
...
test("test_mermaidForFile_whileLoopBecomesADiamondWithABackEdge", () => {
    ...
    assert.equal(
        ...
        'B_sum["sum(list: number[])"]\n' +
        'B_total_equals_0["let total = 0"]\n' +
        'B_total_equals_halve_total["total = halve(total)"]\n' +
        'B_total_plus_equals_x["total += x"]\n' +
        'Q_CHOICE_for_x_of_list_N["i >= list.length"]\n' +
        'Q_CHOICE_for_x_of_list_Y["i < list.length; x = list[i];"]\n' +
...
test("test_mermaidForFile_doWhileLoopWalksTheBodyFirstThenLoopsBackOnTheCondition", () => {
    // Scenario: a do-while loop runs the body once, then loops back while the condition holds.
    const source = 'function sum(list: number[]) {\n  let total = 0;\n  do {\n    total = halve(total);\n  } while (total > 100);\n  return total;\n}\n';
    const result = mermaidForFile("w.ts", source);
    // Verify: the body chains before the diamond; Y loops back to the body; N falls through to the return.
    assert.equal(
        result,
        '%%{init: {"flowchart": {"wrappingWidth": 100000}}}%%\nflowchart TD\n' +
        'B_w_ts["w.ts"]\n' +
...
test("test_mermaidForFile_mapsOperatorWordsKeepsKeywordNamesAndStripsStringQuotesInIds", () => {
    // Reported bug: entry.type !== "user" produced Q_entry_"user" (type dropped, !== dropped, quotes kept).
    const source = 'function f(entry) {\n  if (entry.type !== "user") {\n    return null;\n  }\n}\n';
    const result = mermaidForFile("p.ts", source);
    assert.ok(result.includes("Q_entry_type_not_equals_user{"));
    // Labels escape embedded double-quotes so the renderer accepts them.
    assert.ok(result.includes('Q_entry_type_not_equals_user{"if( entry.type !== #quot;user#quot; )"}'));
});

test("test_mermaidForFile_processExitIsTerminalAndDoesNotLinkToTheNextStatement", () => {
    const source = 'function f() {\n  process.exit(0);\n  doNext();\n}\n';
    const result = mermaidForFile("q.ts", source);
    assert.ok(result.includes("B_f --> B_process_exit_0\n"));
...
test("test_mermaidForFile_encodesBackslashAsEntitySoALiteralNewlineInAStringDoesNotBreakTheLabel", () => {
    ...
    const result = mermaidForFile("r.ts", source);
    assert.ok(result.includes('s.split(#quot;#92;n#quot;)'));
});
