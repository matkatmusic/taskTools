// Full reset of a merged-and-closed task on the "one-submodule" repo shape undoes the merge in root and submodule.
...
test("test_fullReset_merged_oneSubmodule_undoesTheMergeBackToTheRecordedResetPointEverywhere", async () => {
    const taskNumber = 812;
    const fixture = makeShapeFixture("one-submodule", "at-head", taskNumber);
    const { worktree } = spawnTask(fixture.rootPath, taskNumber, "run-1");
    commitWorktreeWork(worktree, submodulePathsByShape["one-submodule"]);
