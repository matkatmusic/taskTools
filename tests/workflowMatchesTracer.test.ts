// Diffs skills/tackle-tasks/tackle-tasks.workflow.js against scripts/tracePipeline.ts for every
// fixture in scripts/tracePipelinePaths.json. The workflow file is not an ES module — it uses
// top-level `return`, the shape the Workflow sandbox expects — so it is evaluated with
// node:vm's compileFunction, the same technique tests/tackle-tasks_bootstrap.workflow.test.ts
// already uses. No experimental flags required.
...
async function runWorkflow(fake: unknown): Promise<string[]> {
