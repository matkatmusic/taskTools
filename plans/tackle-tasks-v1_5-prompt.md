...
**9.** `is the task active?` `Try: mark the task active in tasks.json` [C]
Check active status, and when the task is free mark it active for this run, in **one atomic
read-modify-write**. These are drawn as two boxes but are one operation, because nothing may make the
task active between the question and the write. See `claimTask` in `taskRunState.ts`.
...
**11.** `Try: mark the task active in tasks.json` — ambiguous result [C]
...
**13.** `create a worktree` `take the worktree lease for this run` [C]
...
**14.** `is the worktree safe to use?` [C]
...
**15.** `is the previous run's work resumable?` `adopt the lease for this run` [C]
...
**19.** `init submodules recursively` [C]
...
**20.** `what is the docs mode?` `auto generate docs` `update auto generated docs` [C]
...
**21.** `Input: { worktree, docs mode, clarify request? }` [C]
...
**27.** `CLARIFY` `2 clarify rounds done?` [C]
...
**28.** `write the clarify request into the tasks.json entry` [C]
...
**29.** `2 clarify rounds done?` = YES [C]
...
**37.** `implement task` `agent() errored` [C]
When the agent returns nothing usable, stop with exit type `agent-failed`.

**38.** `commit if needed` [C]
...
by the mechanism in paragraph 22.
...
**41.** `Try: run task tests` `do the task tests pass?` [C]
...
**42.** `have 2 fixes already been attempted?` [C]
...
**43.** `amend tasks.json entry with the failing tests` [C]
...
**44.** `have 2 fixes already been attempted?` = YES [C]
...
**46.** `Input: { plan, tasks.json entry, task test files, implementation diff, test command, test results, pre-existing test files }` [C]
...
**47.** `codex reviews the tests` `agent() errored` [C]
When the agent returns nothing usable, stop with exit type `agent-failed`.

**48.** `are the tests flagged?` = NO [C]
When the tests are not flagged, hand the finished implementation to the rebase preamble pipeline.

**49.** `2 codex test reviews done?` `amend tasks.json entry with codex's notes and fixes` [C]
...
**50.** `2 codex test reviews done?` = YES [C]
...
**51.** `Try: lock the source repo` `acquired?` [C]
...
**52.** `have 15 minutes passed?` `wait 5s` [C]
...
**53.** `have 15 minutes passed?` = YES [C]
...
**55.** *(rebase, suite and merge pipelines)* [C]
...
**56.** `Try: rebase onto the target branch` [C]
...
**57.** `skip every layer the receipt records as already landed` [C]
...
**58.** `did the rebase report conflicts?` `2 conflict fixes done?` `fix conflicts` [S]
...
**59.** `fix conflicts` `agent() errored` [C]
When the agent returns nothing usable, stop with exit type `agent-failed`.

**60.** `commit if needed` `Try: continue the rebase` `is the rebase finished?` [C]
...
**61.** `2 conflict fixes done?` = YES [C]
...
**62.** `Try: run the full suite` `do all tests pass?` [C]
...
**65.** `fix the codebase so the full suite passes` `agent() errored` [C]
When the agent returns nothing usable, stop with exit type `agent-failed`.

**66.** `2 suite fix attempts done?` = YES [C]
...
**67.** `did every change stay inside the task's file fence?` [C]
...
**68.** `did every change stay inside the task's file fence?` = NO [C]
...
**70.** `Try: merge worktrees and submodules, no fast-forward` [C]
...
**71.** `Each layer that lands writes its merge ref AS it lands` [C]
...
**72.** `read the publication state from the layer merge refs` [C]
...
**73.** `what is the publication state?` [C]
...
**75.** `NONE LANDED` `2 merge attempts done?` = NO [C]
...
**76.** `2 merge attempts done?` = YES [C]
...
as a **no-op layer**, not a failure, and paragraph 70 does not apply to it: there is no branch to
...
**79.** `record merge commit hashes to tasks.json` `write exit type completed to tasks.json` [C]
...
**80.** `record modified files to tasks.json` [C]
Record the modified files on the task's run record.

**81.** `clean up worktrees, leases, persistence refs and source lock` [C]
...
**82.** `build the closure note from the recorded run` `mark task inactive in tasks.json` [C]
Build the closure note from what the run actually recorded, then mark the task inactive.

**83.** `move task to completedTasks.json and update tasks blocked by it` `report the closure note` `stop` [C]
Move the task to `completedTasks.json`, update the tasks it was blocking, and report the closure note.

**84.** *(whole tail)* [C]
This tail is **not** what makes a dead run safe. What makes it safe is that the merge wrote its layer
refs before this tail ever ran: the failures exit reads those refs, sees that work landed, and
...
**85.** `read the publication state from the layer merge refs` `did ANY of this task's work land?` [C]
...
**86.** `did ANY of this task's work land?` = YES `write the publication outcome` [C]
...
**87.** `write exit type and exit notes to tasks.json` [C]
When nothing landed, write the incoming exit type and note to the task's run record.

**88.** `record modified files to tasks.json` [C]
Record the modified files. When no worktree was ever created, this records an empty list.

**89.** `does this run still hold the worktree lease?` `release the worktree lease, keep the worktree` [C]
...
**90.** `does this run still hold the source repo lock?` `release the source repo lock` [C]
...
**91.** `mark task inactive in tasks.json` [C]
...
**92.** `report the run's exit type and note` `stop` [C]
...
**96.** `report the exit type and note` `stop` [C]
`invalid-number`, `blocked`, `already-active`, and a `run-failed` raised before the task was marked
active all happen before any run record exists, so there is nothing to write and nothing to release.
Report the exit type and note, and stop.

**97.** *(whole tail)* [C]
`already-active` must **not** touch the run it finds. That run belongs to another invocation, and
writing to it would corrupt a live run.
...
**C. The source lock can go stale inside a long box.** See paragraph 69.
---
...
**2. The rebase does not read the merge receipt.** Paragraph 57 is a diagram rule with no
