...
**5.** `is the task active?` `Try: mark the task active in tasks.json` [C]
...
**7.** `create a worktree` `auto generate docs` `init submodules recursively` [C]
...
**9.** `is the previous run resumable?` [C]
...
**10.** `update docs` `init submodules recursively` [C]
...
**13.** `did the agent return a result?` `retry the box` [C]
...
**14.** `Output` `Receipt: { }` `is the plan file structure valid?` [C]
...
**16.** `Output` `Receipt: { }` `is the review file structure valid?` [C]
...
**17.** `what is the review verdict?` [C]
...
**18.** `SCRAP` `First Time Scrap?` `script adds the codex scrap notes to the task brief` [C]
...
**20.** `AMEND` `Try: script applies codex amendments to the plan` [C]
...
**22.** `ACCEPT` `Output` `Receipt: { }` `is the finished plan receipt structure valid?` [C]
...
**24.** `did the agent return a result?` `retry the box` [C]
When the agent returns nothing, launch the box again.

**25.** `record implementation notes file to tasks.json` [C]
...
**28.** `do the tests fail?` `First fail?` `fix the codebase` [S]
...
**29.** `Output` `Receipt: { }` `is the fix receipt structure valid?` [C]
...
**31.** `codex reviews tests against task details and plan file` [S]
...
**32.** `Output` `Receipt: { }` `is the test review receipt structure valid?` [C]
...
**33.** `are the tests flagged?` `First flagging?` `amend the tests` [C]
...
**34.** `Output` `Receipt: { }` `is the amendment receipt structure valid?` [C]
...
**36.** `can the source repo be locked?` `First time held?` `Try: wait` [C]
...
**37.** `Try: lock the source repo` `did locking the source repo succeed?` `First lock failure?` `Try: wait` [C]
...
**38.** `Output` `Receipt: { }` `is the finished implementation receipt structure valid?` [C]
...
**39.** `Try: rebase onto the target branch if needed` [C]
...
**40.** `did the rebase report conflicts?` `First conflict?` `fix conflicts` [C]
...
**41.** `Output` `Receipt: { }` `is the conflict fix receipt structure valid?` [C]
...
**43.** `commit if needed` `Try: continue replaying commits on top of the target branch` `is the rebase finished?` [C]
...
**44.** `Try: run the full suite` `do all tests pass?` [C]
...
**45.** `First suite failure?` `fix the codebase so the full suite passes` [S]
On the first suite failure, launch an agent to fix the codebase, then commit and rerun.

**46.** `Output` `Receipt: { }` `is the suite fix receipt structure valid?` [C]
...
**48.** `did every change stay inside the task's owned files?` [C]
...
**49.** `Try: merge worktrees and submodules, no fast-forward` `did the merge land?` [C]
...
**53.** `record merge commit hashes to tasks.json` `write exit type completed to tasks.json` `record modified files to tasks.json` [C]
Record the merge commits, the exit type `completed`, and the modified files on the task's run record,
before releasing anything.

**54.** `clean up worktrees, leases, persistence refs and source lock` [C]
...
**55.** `build the closure note from the recorded run` `mark task inactive in tasks.json` [C]
Build the closure note from what the run actually recorded, then mark the task inactive.

**56.** `move task to completedTasks.json and update tasks blocked by it` `report the closure note` `stop` [C]
...
**57.** `write exit type and exit notes to tasks.json` `record modified files to tasks.json` `mark task inactive in tasks.json` [C]
...
**58.** `was a worktree created?` `nothing to release` [C]
When no worktree was created, there is nothing to release.

**59.** `was the source repo locked?` `release the worktree lease` `release the worktree lease and the source lock` [C]
Release the worktree lease, and the source lock too when this run held it. Release only what this run
proved it owns.

**60.** `report the run's exit type and note` `stop` [C]
...
**61.** `report the exit type and note` `stop` [C]
