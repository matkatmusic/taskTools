...
`${t.siblingTasks.length > 0 ? "These tasks share files..." : "No other open task shares files with task " + t.number + "."}`

`${t.blockedBy.length > 0 ? "These tasks block task..." : "No open task blocks task " + t.number + "."}`

`${t.blocks.length > 0 ? "Task ... blocks these tasks..." : "Task " + t.number + " blocks no open task."}`
