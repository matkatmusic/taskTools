...
You are allowed to read every file the read-file skill put into your context, these read-only files: src/a.ts, src/b.ts, plus every file named by a `clarifyRequest` in the brief, and nothing else.
Files named by the brief's `clarifyRequest` count as files you were given.

---- TESTS_FIELD ("skip" means no TDD requirement) ----
node --test tests/thing.test.ts
