...
import { main } from "../../../scripts/steps/pipeline-plan/ARE_2_CLARIFY_ROUNDS_DONE.ts";
...
function makeProjectRoot(clarifyAttempts: number): string {
    const root = mkdtempSync(join(tmpdir(), "are-2-clarify-rounds-done-"));
...
const packet = (projectRoot: string) => JSON.stringify({
    taskNumber: 35, runId: "run-1", worktree: "/tmp/fake-worktree", sourceBranch: "master", projectRoot, clarifyRequest: "which database?",
});

test("test_ARE_2_CLARIFY_ROUNDS_DONE_isNotDoneUnderTheCap", () => {
    const output = main(packet(makeProjectRoot(0)));
    assert.equal(output.next, "WRITE_CLARIFY_REQUEST");
    assert.equal(output.exitType, "");
});

test("test_ARE_2_CLARIFY_ROUNDS_DONE_isDoneAtTheCap", () => {
    const output = main(packet(makeProjectRoot(2)));
    assert.equal(output.next, "EXIT_WORKFLOW_PLAN");
