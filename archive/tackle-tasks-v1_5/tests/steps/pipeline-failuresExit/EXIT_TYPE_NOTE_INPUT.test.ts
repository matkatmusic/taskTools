// Behavioral checks for scripts/steps/pipeline-failuresExit/EXIT_TYPE_NOTE_INPUT.ts.  Run: node --test tests/steps/pipeline-failuresExit/EXIT_TYPE_NOTE_INPUT.test.ts
...
test("test_EXIT_TYPE_NOTE_INPUT_overridesTheIncomingBoxWithItsOwn", () => {
    const packet = {
        box: "EXIT_WORKFLOW_SUITE", scriptSignal: "continue", taskNumber: 169, runId: "run-a",
        projectRoot: "/repo", worktree: "/repo/.worktrees/task-169", sourceBranch: "master",
        exitType: "run-failed", exitNote: "boom",
    };

    const output = main(JSON.stringify(packet));

    assert.equal(output.box, "EXIT_TYPE_NOTE_INPUT");
});
