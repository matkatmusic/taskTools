// The LOCK_SOURCE_REPO box runs inside run-step, so it tries once and never waits.  Run: node --test tests/steps/pipeline-rebasePreamble/LOCK_SOURCE_REPO.test.ts
...
import { main } from "../../../scripts/steps/pipeline-rebasePreamble/LOCK_SOURCE_REPO.ts";
...
const TEMPLATE_PATH = join(dirname(fileURLToPath(import.meta.url)), "../../../scripts/steps/pipeline-rebasePreamble/LOCK_SOURCE_REPO.template.json");
...
test("test_lockSourceRepo_takesTheLockWhenNobodyHoldsIt", () => {
    ...
    const result = main(JSON.stringify({
        runId: "run-a", taskNumber: 1, projectRoot, worktreePath: "/wt", sourceBranch: "main", lockWaitStartedAt: LOCK_WAIT_STARTED_AT,
    }));
...
test("test_lockSourceRepo_returnsAtOnceWhenAnotherRunHoldsTheLock", () => {
    ...
    const result = main(JSON.stringify({ runId: "run-a", taskNumber: 1, projectRoot, lockWaitStartedAt: LOCK_WAIT_STARTED_AT }));
...
test("test_lockSourceRepo_rejectsARelativeProjectRoot", () => {
    assert.throws(() => main(JSON.stringify({ runId: "run-a", taskNumber: 1, projectRoot: "relative/path", lockWaitStartedAt: LOCK_WAIT_STARTED_AT })));
});
