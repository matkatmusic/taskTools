// Behavioral checks for scripts/steps/pipeline-merge/ARE_2_MERGE_ATTEMPTS_DONE.ts. Mutating: raises a persisted counter on an inline tasks.json.
...
import { main } from "../../../scripts/steps/pipeline-merge/ARE_2_MERGE_ATTEMPTS_DONE.ts";
...
const TEMPLATE_PATH = join(
    dirname(fileURLToPath(import.meta.url)), "../../../scripts/steps/pipeline-merge/ARE_2_MERGE_ATTEMPTS_DONE.template.json",
);
...
function packet(taskNumber: number, runId: string, projectRoot: string): string {
    return JSON.stringify({
        worktreePath: "/wt", rootSourceBranch: "main", taskNumber, runId, projectRoot, suiteFixAttempts: 0,
        state: "NONE LANDED", landed: [], notLanded: ["root"], commits: [],
    });
}

test("test_ARE_2_MERGE_ATTEMPTS_DONE_reentersRebaseOnTheFirstAttempt", () => {
    const taskNumber = 1;
    const runId = "run-1";
    const projectRoot = makeProjectRootWithClaimedTask(taskNumber, runId);

    const result = main(packet(taskNumber, runId, projectRoot));

    assert.equal(result.box, "ARE_2_MERGE_ATTEMPTS_DONE");
...
test("test_ARE_2_MERGE_ATTEMPTS_DONE_exitsMergeFailedAfterTwoAttempts", () => {
    const taskNumber = 2;
    const runId = "run-2";
    const projectRoot = makeProjectRootWithClaimedTask(taskNumber, runId);
