...
function buildSuiteFixPrompt(packet: Input): string {
    const root = packet.worktreePath.replace(/\/+$/, "");
    return `## YOUR JOB
...
Return a single field, \`fixSummary\`: one paragraph naming which failures you fixed, and any failure left unaddressed and why.
